...
def create_splats_with_optimizers(
    ...
    counters = torch.zeros(N, dtype=torch.int32)                               # [N,] add by dj, 统计高斯函数被优化次数
    counter = torch.nn.Parameter(counters, requires_grad=False)
    params = [
        # name, value, lr
    ...
    params.append(("counter", counter, 0.0))     # add by dj, 统计高斯函数被优化次数
...
class Runner:
    ...
    def render_traj(self, step: int):
        ...
        camtoworlds_all = self.parser.camtoworlds[5:-5]
        if cfg.render_traj_path == "interp":
            camtoworlds_all = generate_interpolated_path(camtoworlds_all, 15)  # [N, 3, 4]
        elif cfg.render_traj_path == "ellipse":
            height = camtoworlds_all[:, 2, 3].mean()
        ...
        camtoworlds_all = np.concatenate(
            [camtoworlds_all, np.repeat(np.array([[[0, 0, 0, 1]]]), len(camtoworlds_all), axis=0)],
            axis=1,
        )  # [N, 4, 4]
        ...
        num_raw_imgs = len(image_paths)
        num_renders = len(camtoworlds_all)

        canvas_all = []
        for i in tqdm.trange(num_renders, desc="Rendering trajectory"):
            camtoworlds = camtoworlds_all[i : i + 1]
            Ks = K[None]
            ...
            rgb_render = torch.clamp(renders[..., :3], 0.0, 1.0).squeeze(0).cpu().numpy()  # [H, W, 3]
            rgb_render = (rgb_render * 255).astype(np.uint8)

            # === 3. 匹配一个最近的原始图像 ===
            idx_raw = int(i * num_raw_imgs / num_renders)
            idx_raw = min(idx_raw, num_raw_imgs - 1)
            raw_img = imageio.imread(image_paths[idx_raw])  # [H, W, 3]
            if raw_img.shape[2] == 4:
                raw_img = raw_img[..., :3]
                
            # 尺寸对齐
            if raw_img.shape[:2] != rgb_render.shape[:2]:
                raw_img = np.array(Image.fromarray(raw_img).resize((rgb_render.shape[1], rgb_render.shape[0])))

            # 拼接：左为渲染图，右为原始图
            canvas = np.concatenate([rgb_render, raw_img], axis=1)  # [H, 2W, 3]
            canvas_all.append(canvas)
